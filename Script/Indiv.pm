...
sub Indiv{
  ...
  if (!-e $cache) {
	...
	if (!defined ($Cmdline{"phased-vcf"})){
		...
		if (-e $Cmdline{"phased-vcf"} && $Cmdline{"phased-vcf"} =~ /\.vcf*/){
 ...
 my $index_file=$out."/index.html";
   ...
   if (!defined $gene_split[1]){
    ...
    #print Dumper (@choose_coding);
     ...
     if ($in_phased =~ /.gz$/) {
      printf "gzip vcf \n";
      #print $conf::config::vcftools_bin_dir."/vcftools --gzvcf ".$in_phased." --recode --from-bp ".$gene_start." --to-bp ".$gene_stop." --chr ".$chr." --out ".$select_phased;
      ...
      #print $conf::config::vcftools_bin_dir."/vcftools --vcf ".$in_phased." --recode --from-bp ".$gene_start." --to-bp ".$gene_stop." --chr ".$chr." --out ".$select_phased;
     ...
     printf "\n==> END VCFTOOLS \n";
      ...
      my $htlm_sum_file=$gene_dir."/index.html";
        ...
        print HTMLS "<div w3-include-html=\"http://med-laennec.univ-brest.fr/GEMPROT/header.html\" id=\"gemprotheader\" class=\"gemprotheader\"></div>";
        ...
        print HTMLS "<thead><tr><th>Sample</th><th>Haplotype 1</th><th>Haplotype 2</th></tr></thead><tbody>";      
			...
			if ($fich1[6]eq"+") {
				...
				for my $k( sort {$a<=>$b} keys %{$h{$chr}}){
					...
					$h{$chr}{$k}{"amino"}=$aa_pos-($h{$chr}{$k}{"amino"})+1;
      ...
      #print Dumper (\@doma);
        ...
        foreach my $domb (sort keys (%prot_domain)){
          ...
          if (grep { $_ eq $name } @dom_list)  {
            ...
            print HTML  $comptage{$prot_domain{$domb}{"Domain"}}." <a href=\"http://pfam.xfam.org/family/".$prot_domain{$domb}{"Accession"}."\">".$prot_domain{$domb}{"Domain"}."</a> domain(s) <br>";
        ...
        print "Sample ".$nb_sample."   \r";
				...
				if ($fich1[6]eq"+") {
					for my $k( sort {$a<=>$b} keys %{$hap1{$chr}}){
						$sequence1.=$hap1{$chr}{$k};
					}	
					for my $k( sort {$a<=>$b} keys %{$hap2{$chr}}){
						$sequence2.=$hap2{$chr}{$k};
					}
					for my $k( sort {$a<=>$b} keys %{$ref1{$chr}}){
						$sequenceref1.=$ref1{$chr}{$k};
					}
					for my $k( sort {$a<=>$b} keys %{$ref2{$chr}}){
        ...
        #printf $sample." sample\n";
				...
				if (defined $dif_hap{$gene}{$mut_1b}) {
					$dif_hap{$gene}{$mut_1b}{"nb"}+=1;
          $dif_hap{$gene}{$mut_1b}{"Sample"}.= ",".$sample ;
					$nb_hap_tot+=1;
				} 
				else {
					$dif_hap{$gene}{$mut_1b}{"nb"}=1;
          $dif_hap{$gene}{$mut_1b}{"Sample"} = $sample ;
				...
				if (defined $dif_hap{$gene}{$mut_2b}) {
					$dif_hap{$gene}{$mut_2b}{"nb"}+=1;
          $dif_hap{$gene}{$mut_2b}{"Sample"}.= ",".$sample ;
					$nb_hap_tot+=1;
				} 
				else {
					$dif_hap{$gene}{$mut_2b}{"nb"}=1;
      ...
      my $hm;
			foreach $hm (keys %{$dif_hap{$gene}}) {
				#print $gene."\t".$hm."\t";
				print GMF $gene."\t".$hm."\t".$dif_hap{$gene}{$hm}."\t".$dif_hap{$gene}{$hm}{"nb"}/$nb_hap_tot."\t".$dif_hap{$gene}{$hm}{"Sample"}."\n";
      ...
      my $hp;
			foreach $hp (keys %{$dif_hap{$gene}}) {
				#print $gene."\t".$hm."\t";
        my $freq = sprintf("%.3g", $dif_hap{$gene}{$hp}{"nb"}/$nb_hap_tot);
        ...
        my @sample_split = split(/,/, $dif_hap{$gene}{$hp}{"Sample"});
      ...
      print HTMLS "</tbody></table></div>";
      print HTMLS "<div w3-include-html=\"http://med-laennec.univ-brest.fr/GEMPROT/footer.html\" id=\"gemprotfooter\" class=\"gemprotfooter\"></div>";
