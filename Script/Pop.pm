      ...
      foreach my $mot ( @doma ) {
        ...
        print "Sample ".$nb_sample."   \r";
             ...
             $dif_hap{$gene}{$mut_1b}{$locity}=0;
				...
				$dif_hap{$gene}{$mut_1b}{$city}+=1;
        my $hap_mut_file = $tmp_dir.$gene.".mutation_by_hap_".$mut_1b.".txt";
				...
				if ($mut_info1 ne "no_mutations"){
					for my $m (split ",", $mut_info1){
						...
						print HMF $mut_info1.",hap1\t".$gene."\t".$lmut[1]."\t".$lmut[0]."\t".$lmut[2]."\t".$lmut[3]."\t".$lmut[4]."\tblue2\n";
					}
				 
				}
        system("Rscript ".$rep_script."/Plot_new_annotation.R -m ".$hap_mut_file." -d ".$dom_file." -l ".$aa_pos." -p ".$V_dir."/".$gene."_".$mut_1b." -s yes -a pop > /dev/null 2>&1");
        ...
        close HMF;
             ...
             chomp $locity;
					...
					$dif_hap{$gene}{$mut_2b}{$city}+=1;
          my $hap_mut_file = $tmp_dir.$gene.".mutation_by_hap_".$mut_2b.".txt";
				  open HMF, ">".$hap_mut_file;
				  print HMF "Hap\tGene\tPosition\tRef\tAlt\tType\tStyle\tColor\n";
