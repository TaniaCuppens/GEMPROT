...
sub AlleleOrigin{
	...
	my(%o)=('0' => 'unknown', '1' => 'germline', '3' => '3?', '2' => 'somatic', '4' => 'inherited', '8' => 'paternal', '9' => '9?', '16' => 'maternal', '32' => 'de-novo', '64' => 'biparental', '128' => 'uniparental', '256'
 => 'not-tested', '512' => 'tested-inconclusive', '1073741824' => 'other');
	if(exists $o{$id_origin}){
	   	return $o{$id_origin};
	}
	else{
...
sub VariantClinicalSignificance{
	my($id_sign)=@_;
	$id_sign=uc $id_sign;
	my(%c)=('0' => 'Uncertain significance', '1' => 'not provided', '2' => 'Benign', '3' => 'Likely benign', '4' => 'Likely pathogenic', '5' => 'Pathogenic', '6' => 'drug response', '7' => 'histocompatibility', '255' => 'other');
	if (looks_like_number($id_sign) && exists $c{$id_sign}){
	   	return $c{$id_sign};
	}
	else{
	    return $id_sign;
...
sub codon2aa{
	my($codon)=@_;
	$codon=uc $codon;
	my(%g)=('TCA'=>'S','TCC'=>'S','TCG'=>'S','TCT'=>'S','TTC'=>'F','TTT'=>'F','TTA'=>'L','TTG'=>'L',
...
sub prot_modif_type {
  ...
  my $genomique=0;
	...
	for (my $i=0;$i<length($sequence_protref);$i++) {
  ...
  # print $pos."position###\n";
   if (defined $mut_hash_aa{$nhap}{$pos}){
      ...
      $hash_mut_type{$pos}{"genomique"}=$genomique;
   #  print $mut_hash_aa{$nhap}{$pos}." phase\n";
     if ($mut_hash_aa{$nhap}{$pos}{"phase"} eq "missing"){
         $hash_mut_type{$pos}{"color"}="red"; 
         $hash_mut_type{$pos}{"type"}="missing";
         $hash_mut_type{$pos}{"frameshift"}=0;
         $hash_mut_type{$pos}{"ref"}=".";
			  $hash_mut_type{$pos}{"mut"}=".";
        $hash_mut_type{$pos}{"symbol"}=5;
        $hash_mut_type{$pos}{"genomique"}=$genomique;
        $nb_diff++;
         next;
     }
     elsif ($nhap eq "hap1" && $mut_hash_aa{$nhap}{$pos}{"phase"} eq "yes") {
       $color="blue2";
       $hash_mut_type{$pos}{"color"}="blue2";
    }
    elsif ($nhap eq "hap2" && $mut_hash_aa{$nhap}{$pos}{"phase"} eq "yes") {
        $color="indianred3";
       $hash_mut_type{$pos}{"color"}="orangered2";
    }
    else {
        $color="azure4";
       $hash_mut_type{$pos}{"color"}="azure4"; 
    }
    #print $hash_mut_type{$pos}{"color"}." couleur\n";
    }
    else {
       $hash_mut_type{$pos}{"genomique"}=$genomique;
       $hash_mut_type{$pos}{"color"}=$color;
    }
    $hash_mut_type{$pos}{"frameshift"}=0;
		...
		if ($aaref eq $aamut) {
			...
			if ($syn eq 'yes' && defined $mut_hash_aa{$nhap}{$pos}){
     #   print $mut_hash_aa{$nhap}{$pos}."\n";
				$hash_mut_type{$pos}{"type"}="synonymous";
        $hash_mut_type{$pos}{"symbol"}=1;
				$nb_diff++;
			}
			else {
				$hash_mut_type{$pos}{"type"}="match";
			}
		}
		elsif ($aaref eq '*') {
			$hash_mut_type{$pos}{"ref"}="*";
			$hash_mut_type{$pos}{"mut"}=$aamut;
			$hash_mut_type{$pos}{"type"}="elongation";
			$hash_mut_type{$pos}{"symbol"}=13;
      $i=length($sequence_protref);
			...
			$hash_mut_type{$pos}{"ref"}=$aaref;
			$hash_mut_type{$pos}{"mut"}="*";
			$hash_mut_type{$pos}{"type"}="stop";
			$hash_mut_type{$pos}{"symbol"}=8;
			$i=length($sequence_protref);
			$nb_diff++;
		}
		elsif ($aamut eq '-') {
			$hash_mut_type{$pos}{"ref"}=$aaref;
			$hash_mut_type{$pos}{"mut"}="del";
			$hash_mut_type{$pos}{"type"}="deletion";
			$hash_mut_type{$pos}{"symbol"}=17;
      if ($hash_mut_type{$pos-1}{"type"}eq"synonymous") {
          $hash_mut_type{$pos-1}{"type"}="match"; 
      }
			$nb_diff++;
		}
		elsif ($aaref eq '-') {
			if (($pos-1)==0){
				$hash_mut_type{($pos-1)}{"mut"}=$hash_mut_type{($pos-1)}{"mut"}.$aamut;
				$hash_mut_type{($pos-1)}{"type"}="insertion";
				$hash_mut_type{$pos-1}{"ref"}="ins";
				$hash_mut_type{$pos-1}{"symbol"}=6;
				$nb_diff++;
			}
			else {
				$hash_mut_type{($pos-1)}{"mut"}=$hash_mut_type{($pos-1)}{"mut"}.$aamut;
        ...
        $hash_mut_type{$pos-1}{"ref"}="ins";
			...
			$hash_mut_type{$pos}{"ref"}=$aaref; 
			$hash_mut_type{$pos}{"mut"}=$aamut;
			$hash_mut_type{$pos}{"type"}="mismatch";
			$hash_mut_type{$pos}{"symbol"}=19;
      if (defined $hash_mut_type{$pos-1}{"type"}){
			  if ($hash_mut_type{$pos-1}{"type"}eq"mismatch" && !defined $mut_hash_aa{$nhap}{$pos}) {
			  	$hash_mut_type{$pos}{"frameshift"}=$hash_mut_type{$pos-1}{"frameshift"}+1;

			  	if ($hash_mut_type{$pos-1}{"frameshift"}==0) {
				  	$hash_mut_type{$pos-1}{"frameshift"}=-1;
				  }
			  }
        elsif ($hash_mut_type{$pos-2}{"type"}eq"mismatch" && $hash_mut_type{$pos-2}{"type"}ne"synonymous" && !defined $mut_hash_aa{$nhap}{$pos}) {
          $hash_mut_type{$pos-1}{"frameshift"}=$hash_mut_type{$pos-2}{"frameshift"}+1;
				  $hash_mut_type{$pos}{"frameshift"}=$hash_mut_type{$pos-2}{"frameshift"}+2;
		...
		for my $k( sort {$a<=>$b} keys %hash_mut_type){
			...
			if ($hash_mut_type{$k}{"type"} ne "match"){
				...
				if ($hash_mut_type{$k}{"frameshift"}!=0 && $hash_mut_type{$k}{"frameshift"}!=-1){
					...
					$fsize=$hash_mut_type{$k}{"frameshift"};
				}
				if ($hash_mut_type{$k}{"frameshift"}==-1 || $hash_mut_type{$k}{"frameshift"}==0) {
					if ($fsize!=0) {
             
						if ($haplo eq "" && $prev_k !=0){
							$info_haplo .= "FS".$fsize.":".($prev_k-$fsize).":".$prev_k.":frameshift:19:".$hash_mut_type{$k}{"color"};
					...
					if ($haplo eq "" && $hash_mut_type{$k}{"frameshift"}!=-1){
						$info_haplo = $hash_mut_type{$k}{"ref"}.":".$k.":".$hash_mut_type{$k}{"mut"}.":".$hash_mut_type{$k}{"type"}.":".$hash_mut_type{$k}{"symbol"}.":".$hash_mut_type{$k}{"color"};
						$haplo = $hash_mut_type{$k}{"ref"}.":".$k.":".$hash_mut_type{$k}{"mut"};
					}
					elsif ($hash_mut_type{$k}{"frameshift"}!=-1) {
 ...
 for my $m (sort {$a<=>$b} keys (%hash_mut_type)){
...
sub html_file {
 ...
 #print Dumper(\%mut_hash);
   ...
   my $prev_gen=0;
   my $stop_gen_pos="no";
		for my $m (sort {$a<=>$b} keys %{hash_mut_type1}) { 
			if ($hash_mut_type1{$m}{"type"} ne "match"){

        if ($hash_mut_type1{$m}{"genomique"} !=$prev_gen){
          print HTML "<br><li>".$mut_hash{$chr}{$hash_mut_type1{$m}{"genomique"}}{'hap1'}{'vcf'}." amino acid position ".$m." \n</li><br><blockquote>";
          $stop_gen_pos="no";
        }
        
				my $frame= $mut_hash{$chr}{$hash_mut_type1{$m}{"genomique"}}{'hap1'}{'amino'};
        if($stop_gen_pos eq "yes"){
          ...
          $stop_gen_pos="yes";
					while ($hash_mut_type1{$frame}{"type"} ne "elongation" && $hash_mut_type1{$frame}{"type"} ne "stop" && $frame != $aa_pos){	
 
						$frame++;	
					}
					if ($hash_mut_type1{$frame}{"type"} eq "elongation") {
                #print HTML "<br><li>".$mut_hash{$chr}{$hash_mut_type1{$m}{"genomique"}}{'hap1'}{'vcf'}." amino acid position ".$m." \n</li><br><blockquote>";
						print HTML "Frameshift mutation that results in protein elongation <br>";
					}
					elsif ($hash_mut_type1{$frame}{"type"} eq "stop") {
                #print HTML "<br><li>".$mut_hash{$chr}{$hash_mut_type1{$m}{"genomique"}}{'hap1'}{'vcf'}." amino acid position ".$m." \n</li><br><blockquote>";
        ...
        elsif ($hash_mut_type1{$m}{"type"} eq "missing") {
          print HTML "Missing Data<br>";
        }
				elsif ($hash_mut_type1{$m}{"type"} eq "mismatch") {
					my $aapref=aa2properties($hash_mut_type1{$m}{"ref"});
					my $aapmut=aa2properties($hash_mut_type1{$m}{"mut"});
				#	print HTML "<br><li>".$mut_hash{$chr}{$hash_mut_type1{$m}{"genomique"}}{'hap1'}{'vcf'}." amino acid position ".$m." \n</li><br><blockquote>";
					print HTML $hash_mut_type1{$m}{"ref"}."=>".$hash_mut_type1{$m}{"mut"}." ".$hash_mut_type1{$m}{"type"}." mutation<br>";
              ...
              # print HTML "<br><li>".$mut_hash{$chr}{$hash_mut_type1{$m}{"genomique"}}{'hap1'}{'vcf'}." amino acid position ".$m." \n</li><br><blockquote>";
				...
				if(exists( $clin_hash{$chr}{$hash_mut_type1{$m}{"genomique"}})){
					print HTML " ClinVar Information at this position : \n<br>";
					print HTML "<blockquote>";
					foreach my $clin_info (keys %{ $clin_hash{$chr}{$hash_mut_type1{$m}{"genomique"}}}){
						print HTML "<ul class=\"ulclinvar\">";
						#if ($clin_hash{$chr}{$hash_mut_type1{$m}{"genomique"}}{$clin_info}{"ALT"} =~ /HGVS/){
							print HTML "<li>".$clin_hash{$chr}{$hash_mut_type1{$m}{"genomique"}}{$clin_info}{"RS"}."   ".$clin_hash{$chr}{$hash_mut_type1{$m}{"genomique"}}{$clin_info}{"NC"}."   ".$clin_hash{$chr}{$hash_mut_type1{$m}{"genomique"}}{$clin_info}{"CLNSIG"}."   ".$clin_hash{$chr}{$hash_mut_type1{$m}{"genomique"}}{$clin_info}{"CLNDBN"}." <a href=\"https://www.ncbi.nlm.nih.gov/clinvar/variation/".$clin_hash{$chr}{$hash_mut_type1{$m}{"genomique"}}{$clin_info}{"ID"}."\">"."see clinvar"."</a> </li><br>";
    ...
    my $prev_gen2=0;
    my $stop_gen_pos2="no";
		for my $m2 (sort {$a<=>$b} keys %{hash_mut_type2}) {
			if ($hash_mut_type2{$m2}{"type"} ne "match"){
        if ($hash_mut_type2{$m2}{"genomique"} !=$prev_gen2){
          print HTML "<br><li>".$mut_hash{$chr}{$hash_mut_type2{$m2}{"genomique"}}{'hap2'}{'vcf'}." amino acid position ".$m2." \n</li><br><blockquote>";
          $stop_gen_pos2="no";
        }
        my $frame2= $mut_hash{$chr}{$hash_mut_type2{$m2}{"genomique"}}{'hap2'}{'amino'};
        if($stop_gen_pos2 eq "yes"){
          next;
	 		  ...
	 		  elsif ($hash_mut_type2{$m2}{"frameshift"} == -1) {
          $stop_gen_pos2="yes";
        while ($hash_mut_type2{$frame2}{"type"} ne "elongation" && $hash_mut_type2{$frame2}{"type"} ne "stop" && $frame2 != $aa_pos){	
     
						$frame2++;	
					}
					
					if ($hash_mut_type2{$frame2}{"type"} eq "elongation") {
                #print HTML "<br><li>".$mut_hash{$chr}{$hash_mut_type1{$m}{"genomique"}}{'hap1'}{'vcf'}." amino acid position ".$m." \n</li><br><blockquote>";
						print HTML "Frameshift mutation that results in protein elongation <br>";
					}
					elsif ($hash_mut_type2{$frame2}{"type"} eq "stop") {
                #print HTML "<br><li>".$mut_hash{$chr}{$hash_mut_type1{$m}{"genomique"}}{'hap1'}{'vcf'}." amino acid position ".$m." \n</li><br><blockquote>";
        ...
        elsif ($hash_mut_type2{$m2}{"type"} eq "missing") {
          print HTML "Missing Data<br>";
        }
				elsif ($hash_mut_type2{$m2}{"type"} eq "mismatch") {
					my $aapref=aa2properties($hash_mut_type2{$m2}{"ref"});
					my $aapmut=aa2properties($hash_mut_type2{$m2}{"mut"});
				#	print HTML "<br><li>".$mut_hash{$chr}{$hash_mut_type1{$m}{"genomique"}}{'hap1'}{'vcf'}." amino acid position ".$m." \n</li><br><blockquote>";
					print HTML $hash_mut_type2{$m2}{"ref"}."=>".$hash_mut_type2{$m2}{"mut"}." ".$hash_mut_type2{$m2}{"type"}." mutation<br>";
					print HTML "Change from ".$aapref."amino acid to ".$aapmut."<br>";
				}
				else {
              # print HTML "<br><li>".$mut_hash{$chr}{$hash_mut_type1{$m}{"genomique"}}{'hap1'}{'vcf'}." amino acid position ".$m." \n</li><br><blockquote>";
				...
				if(exists( $clin_hash{$chr}{$hash_mut_type2{$m2}{"genomique"}})){
					print HTML " ClinVar Information at this position : \n<br>";
					print HTML "<blockquote>";
					foreach my $clin_info (keys %{ $clin_hash{$chr}{$hash_mut_type2{$m2}{"genomique"}}}){
						print HTML "<ul class=\"ulclinvar\">";
						#if ($clin_info =~ /HGVS/){
							print HTML "<li>".$clin_hash{$chr}{$hash_mut_type2{$m2}{"genomique"}}{$clin_info}{"RS"}."   ".$clin_hash{$chr}{$hash_mut_type2{$m2}{"genomique"}}{$clin_info}{"NC"}."   ".$clin_hash{$chr}{$hash_mut_type2{$m2}{"genomique"}}{$clin_info}{"CLNSIG"}."   ".$clin_hash{$chr}{$hash_mut_type2{$m2}{"genomique"}}{$clin_info}{"CLNDBN"}." <a href=\"https://www.ncbi.nlm.nih.gov/clinvar/variation/".$clin_hash{$chr}{$hash_mut_type2{$m2}{"genomique"}}{$clin_info}{"ID"}."\">"."see clinvar"."</a>  </li><br>";
	...
	print HTML "</p>";
	print HTML "<br><object class=\"bigpdf\" data=\"".$pdf."\" type=\"application/pdf\">
  alt : <a href=\"".$pdf."\">protein.pdf</a>
</object>"; 
...
sub get_url_pfam {
  ...
  open(GGF, "grep -w \"".$gene."\" ".$pfam_store_file." |");
    ...
    if (@empty != 0){
      $prot_domain{$start}{"Start"} = 1;
   		$prot_domain{$start}{"End"} = 1;
   		$prot_domain{$start}{"Domain"} = "No_domains";
			$prot_domain{$start}{"Accession"} = 1;
  ...
  # printf "curl --silent -LH 'Expect:' -F output=xml 'http://pfam.xfam.org$res_url' ";
    ...
    my $domain_gene="";
		foreach my $match (@entry4){
			...
			foreach my $loc (@entry5){
				...
				$prot_domain{$start}{"Start"} = $loc->getAttribute( 'ali_start' );
   				$prot_domain{$start}{"End"} = $loc->getAttribute( 'ali_end' );
   				$prot_domain{$start}{"Domain"} = $match->getAttribute( 'id' );
				$prot_domain{$start}{"Accession"} = $match->getAttribute( 'accession' );
   ...
   foreach my $dom (@genedomain) {
     my @d_line= split (/\t/ , $dom);
     my $start = $d_line[2];
       $prot_domain{$start}{"Start"} = $d_line[2];
   				$prot_domain{$start}{"End"} = $d_line[3];
   				$prot_domain{$start}{"Domain"} = $d_line[1];
				$prot_domain{$start}{"Accession"} = $d_line[4];
...
sub domain_file {
	...
	foreach my $key (sort {$a <=> $b} keys %prot_domain) {
		print DFILE $gene."\t".$prot_domain{$key}{"Domain"}."\t".$prot_domain{$key}{"Start"}."\t".$prot_domain{$key}{"End"}."\n";
	}
	close DFILE;
}	
...
sub get_header {
	...
	if (defined $header{$col_name}){
		my $num_col=$header{$col_name};
		return $num_col;
	}
	else {
		my $num_col=0;
...
sub parsevcf2{
    ...
    foreach my $line (@variant) {
      ...
      if($geno[0] =~ m/\./){
        $mut_nb1++;
        $mut_nb2++;
        $mut_hash_pos{"hap1"}{$seq_hash_ref{$fich1[0]}{$variant_pos+$i}{"amino"}}{"phase"}="missing"; 
        $mut_hash_pos{"hap1"}{$seq_hash_ref{$fich1[0]}{$variant_pos+$i}{"amino"}}{"vcf"}=$fich1[1];                 
        $mut_hash{$fich1[0]}{$variant_pos+$i}{"hap1"}{"vcf"}=$fich1[0].":".$fich1[1]." ".$fich1[3].">".$fich1[4];    
			  $mut_hash{$fich1[0]}{$variant_pos+$i}{"hap1"}{"nt"}=$fich1[3].">".$fich1[4];
			  $mut_hash{$fich1[0]}{$variant_pos+$i}{"hap1"}{"amino"}=$seq_hash_ref{$fich1[0]}{$variant_pos+$i}{"amino"};
				$mut_hash{$fich1[0]}{$variant_pos+$i}{"hap1"}{"exon"}=$seq_hash_ref{$fich1[0]}{$variant_pos+$i}{"exon"};
				$mut_hash{$fich1[0]}{$variant_pos+$i}{"hap1"}{"nt_pos"}=$seq_hash_ref{$fich1[0]}{$variant_pos+$i}{"nt_pos"};
        $mut_hash_pos{"hap2"}{$seq_hash_ref{$fich1[0]}{$variant_pos+$i}{"amino"}}{"phase"}="missing"; 
        $mut_hash_pos{"hap2"}{$seq_hash_ref{$fich1[0]}{$variant_pos+$i}{"amino"}}{"vcf"}=$fich1[1];                 
        $mut_hash{$fich1[0]}{$variant_pos+$i}{"hap2"}{"vcf"}=$fich1[0].":".$fich1[1]." ".$fich1[3].">".$fich1[4];    
			  $mut_hash{$fich1[0]}{$variant_pos+$i}{"hap2"}{"nt"}=$fich1[3].">".$fich1[4];
			  $mut_hash{$fich1[0]}{$variant_pos+$i}{"hap2"}{"amino"}=$seq_hash_ref{$fich1[0]}{$variant_pos+$i}{"amino"};
				$mut_hash{$fich1[0]}{$variant_pos+$i}{"hap2"}{"exon"}=$seq_hash_ref{$fich1[0]}{$variant_pos+$i}{"exon"};
				$mut_hash{$fich1[0]}{$variant_pos+$i}{"hap2"}{"nt_pos"}=$seq_hash_ref{$fich1[0]}{$variant_pos+$i}{"nt_pos"}; 
        ...
        #print $i." i ".(length($fich1[3])-1)."\n";
            ...
            my $i_inf=$i;                                 
						...
						#	if ($REF_nt[$i] ne $ALT_nt[$i]){
								...
								$seq_hash_hap1{$fich1[0]}{$variant_pos+$i}=$ALT_nt[$i];
								$mut_hash_pos{"hap1"}{$seq_hash_ref{$fich1[0]}{$variant_pos+$i}{"amino"}}{"phase"}=$phased; 
                $mut_hash_pos{"hap1"}{$seq_hash_ref{$fich1[0]}{$variant_pos+$i}{"amino"}}{"vcf"}=$fich1[1];                 
                $mut_hash{$fich1[0]}{$variant_pos+$i}{"hap1"}{"vcf"}=$fich1[0].":".$fich1[1]." ".$fich1[3].">".$ALT;    
								$mut_hash{$fich1[0]}{$variant_pos+$i}{"hap1"}{"nt"}=$REF_nt[$i].">".$ALT_nt[$i];
								$mut_hash{$fich1[0]}{$variant_pos+$i}{"hap1"}{"amino"}=$seq_hash_ref{$fich1[0]}{$variant_pos+$i}{"amino"};
								$mut_hash{$fich1[0]}{$variant_pos+$i}{"hap1"}{"exon"}=$seq_hash_ref{$fich1[0]}{$variant_pos+$i}{"exon"};
								$mut_hash{$fich1[0]}{$variant_pos+$i}{"hap1"}{"nt_pos"}=$seq_hash_ref{$fich1[0]}{$variant_pos+$i}{"nt_pos"}; 
                  ...
                  if(!defined $mut_hash{$fich1[0]}{$variant_pos+$i_inf}{"hap1"}{"nt"}){
                    $mut_hash{$fich1[0]}{$variant_pos+$i_inf}{"hap1"}{"vcf"}=$fich1[0].":".$fich1[1]." ".$fich1[3].">".$ALT;
                    $mut_hash{$fich1[0]}{$variant_pos+$i_inf}{"hap1"}{"nt"}=$REF_nt[$i_inf].">".$ALT_nt[$i_inf];
                    $mut_hash{$fich1[0]}{$variant_pos+$i_inf}{"hap1"}{"amino"}=$seq_hash_ref{$fich1[0]}{$variant_pos+$i_inf}{"amino"};
								    $mut_hash{$fich1[0]}{$variant_pos+$i_inf}{"hap1"}{"exon"}=$seq_hash_ref{$fich1[0]}{$variant_pos+$i_inf}{"exon"};
								    $mut_hash{$fich1[0]}{$variant_pos+$i_inf}{"hap1"}{"nt_pos"}=$seq_hash_ref{$fich1[0]}{$variant_pos+$i_inf}{"nt_pos"}; 
                  }
							#print "\n c'est une insertion ";
              $seq_hash_ref1{$fich1[0]}{$variant_pos+$i_inf}.="-";   
              $mut_hash_pos{"hap1"}{$seq_hash_ref{$fich1[0]}{$variant_pos+$i_inf}{"amino"}}{"phase"}=$phased;  
              $mut_hash_pos{"hap1"}{$seq_hash_ref{$fich1[0]}{$variant_pos+$i_inf}{"amino"}}{"vcf"}=$fich1[1];           
              $seq_hash_hap1{$fich1[0]}{$variant_pos+$i_inf}=$seq_hash_hap1{$fich1[0]}{$variant_pos+$i_inf}.$ALT_nt[$i];                
							$mut_hash{$fich1[0]}{$variant_pos+$i_inf}{"hap1"}{"nt"}=$mut_hash{$fich1[0]}{$variant_pos+$i_inf}{"hap1"}{"nt"}.$ALT_nt[$i];
              ...
              if ($i<=((length($ALT))-1)) {
                $mut_hash{$fich1[0]}{$variant_pos+$i}{"hap1"}{"vcf"}=$fich1[0].":".$fich1[1]." ".$fich1[3].">".$ALT; 
						...
						#	if (!exists $ALT_nt[$i]){	
								...
								$seq_hash_hap1{$fich1[0]}{$variant_pos+$i}="-";
								$mut_hash_pos{"hap1"}{$seq_hash_ref{$fich1[0]}{$variant_pos+$i}{"amino"}}{"phase"}=$phased;
                $mut_hash_pos{"hap1"}{$seq_hash_ref{$fich1[0]}{$variant_pos+$i}{"amino"}}{"vcf"}=$fich1[1];
                $mut_hash{$fich1[0]}{$variant_pos+$i}{"hap1"}{"vcf"}=$fich1[0].":".$fich1[1]." ".$fich1[3].">".$ALT; 
								$mut_hash{$fich1[0]}{$variant_pos+$i}{"hap1"}{"nt"}=$REF_nt[$i].">-";
								$mut_hash{$fich1[0]}{$variant_pos+$i}{"hap1"}{"amino"}=$seq_hash_ref{$fich1[0]}{$variant_pos+$i}{"amino"};
								$mut_hash{$fich1[0]}{$variant_pos+$i}{"hap1"}{"exon"}=$seq_hash_ref{$fich1[0]}{$variant_pos+$i}{"exon"};
								$mut_hash{$fich1[0]}{$variant_pos+$i}{"hap1"}{"nt_pos"}=$seq_hash_ref{$fich1[0]}{$variant_pos+$i}{"nt_pos"}; 
              }                                                                                                                                                  
        ...
        #print $i." i ".(length($fich1[3])-1)."\n";
              ...
              my $i_inf=$i;                            
						#	print "\n c'est un SNP ";
							#if ($REF_nt[$i] ne $ALT_nt[$i]){
								$seq_hash_hap2{$fich1[0]}{$variant_pos+$i}=$ALT_nt[$i];
								$mut_hash_pos{"hap2"}{$seq_hash_ref{$fich1[0]}{$variant_pos+$i}{"amino"}}{"phase"}=$phased; 
                $mut_hash_pos{"hap2"}{$seq_hash_ref{$fich1[0]}{$variant_pos+$i}{"amino"}}{"vcf"}=$fich1[1];  
                $mut_hash{$fich1[0]}{$variant_pos+$i}{"hap2"}{"vcf"}=$fich1[0].":".$fich1[1]." ".$fich1[3].">".$ALT;  
								$mut_hash{$fich1[0]}{$variant_pos+$i}{"hap2"}{"nt"}=$REF_nt[$i].">".$ALT_nt[$i];
								$mut_hash{$fich1[0]}{$variant_pos+$i}{"hap2"}{"amino"}=$seq_hash_ref{$fich1[0]}{$variant_pos+$i}{"amino"};
								$mut_hash{$fich1[0]}{$variant_pos+$i}{"hap2"}{"exon"}=$seq_hash_ref{$fich1[0]}{$variant_pos+$i}{"exon"};
								$mut_hash{$fich1[0]}{$variant_pos+$i}{"hap2"}{"nt_pos"}=$seq_hash_ref{$fich1[0]}{$variant_pos+$i}{"nt_pos"}; 
							#}
                  ...
                  if(!defined $mut_hash{$fich1[0]}{$variant_pos+$i_inf}{"hap2"}{"nt"}){
              $mut_hash_pos{"hap2"}{$seq_hash_ref{$fich1[0]}{$variant_pos+$i_inf}{"amino"}}{"phase"}=$phased;
              $mut_hash_pos{"hap2"}{$seq_hash_ref{$fich1[0]}{$variant_pos+$i_inf}{"amino"}}{"vcf"}=$fich1[1]; 
                  $mut_hash{$fich1[0]}{$variant_pos+$i_inf}{"hap2"}{"vcf"}=$fich1[0].":".$fich1[1]." ".$fich1[3].">".$ALT;      
                    $mut_hash{$fich1[0]}{$variant_pos+$i_inf}{"hap2"}{"nt"}=$REF_nt[$i_inf].">".$ALT_nt[$i_inf];
                    $mut_hash{$fich1[0]}{$variant_pos+$i_inf}{"hap2"}{"amino"}=$seq_hash_ref{$fich1[0]}{$variant_pos+$i_inf}{"amino"};
			        $mut_hash{$fich1[0]}{$variant_pos+$i_inf}{"hap2"}{"exon"}=$seq_hash_ref{$fich1[0]}{$variant_pos+$i_inf}{"exon"};
							$mut_hash{$fich1[0]}{$variant_pos+$i_inf}{"hap2"}{"nt_pos"}=$seq_hash_ref{$fich1[0]}{$variant_pos+$i_inf}{"nt_pos"}; 
                  }
							#print "\n c'est une insertion ";
              $seq_hash_ref2{$fich1[0]}{$variant_pos+$i_inf}.="-"; 
              $mut_hash_pos{"hap2"}{$seq_hash_ref{$fich1[0]}{$variant_pos+$i_inf}{"amino"}}{"phase"}=$phased;  
              $seq_hash_hap2{$fich1[0]}{$variant_pos+$i_inf}=$seq_hash_hap1{$fich1[0]}{$variant_pos+$i_inf}.$ALT_nt[$i]; 
							$mut_hash{$fich1[0]}{$variant_pos+$i_inf}{"hap2"}{"nt"}=$mut_hash{$fich1[0]}{$variant_pos+$i_inf}{"hap2"}{"nt"}.$ALT_nt[$i];
              ...
              if ($i<=((length($ALT))-1)) {
                $mut_hash{$fich1[0]}{$variant_pos+$i}{"hap2"}{"vcf"}=$fich1[0].":".$fich1[1]." ".$fich1[3].">".$ALT; 
								$mut_hash{$fich1[0]}{$variant_pos+$i}{"hap2"}{"nt"}=$REF_nt[$i].$REF_nt[$i].">".$ALT_nt[$i];
								$mut_hash{$fich1[0]}{$variant_pos+$i}{"hap2"}{"amino"}=$seq_hash_ref{$fich1[0]}{$variant_pos+$i}{"amino"};
								$mut_hash{$fich1[0]}{$variant_pos+$i}{"hap2"}{"exon"}=$seq_hash_ref{$fich1[0]}{$variant_pos+$i}{"exon"};
								$mut_hash{$fich1[0]}{$variant_pos+$i}{"hap2"}{"nt_pos"}=$seq_hash_ref{$fich1[0]}{$variant_pos+$i}{"nt_pos"}; 
              }
						...
						#	if (!exists $ALT_nt[$i]){	
								...
								$seq_hash_hap2{$fich1[0]}{$variant_pos+$i}="-";
								$mut_hash_pos{"hap2"}{$seq_hash_ref{$fich1[0]}{$variant_pos+$i}{"amino"}}{"phase"}=$phased;
                $mut_hash_pos{"hap2"}{$seq_hash_ref{$fich1[0]}{$variant_pos+$i}{"amino"}}{"vcf"}=$fich1[1];
                $mut_hash{$fich1[0]}{$variant_pos+$i}{"hap2"}{"vcf"}=$fich1[0].":".$fich1[1]." ".$fich1[3].">".$ALT; 
								$mut_hash{$fich1[0]}{$variant_pos+$i}{"hap2"}{"nt"}=$REF_nt[$i].">-";
								$mut_hash{$fich1[0]}{$variant_pos+$i}{"hap2"}{"amino"}=$seq_hash_ref{$fich1[0]}{$variant_pos+$i}{"amino"};
								$mut_hash{$fich1[0]}{$variant_pos+$i}{"hap2"}{"exon"}=$seq_hash_ref{$fich1[0]}{$variant_pos+$i}{"exon"};
								$mut_hash{$fich1[0]}{$variant_pos+$i}{"hap2"}{"nt_pos"}=$seq_hash_ref{$fich1[0]}{$variant_pos+$i}{"nt_pos"}; 
             }                                                                                                                                                   
