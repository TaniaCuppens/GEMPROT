...
sub checkfileconfig {
    ...
    my $vcft   = "";
    open( REFG,
        "ls -l " . $conf::config::genome_reference . "*.fai 2> /dev/null | " );
    $refg .= <REFG>;
...
sub aa2properties {
    ...
    my (%p) = (
        'G' =>
          'smallest amino acid, aliphatic, no charge, not hydrophilic (0.67)',
        'A' => 'no charge, aliphatic, hydrophobic (1.0)',
        'V' => 'no charge, aliphatic, hydrophobic (2.3) ',
        'L' =>
          'no charge, aliphatic, hydrophobic (2.2), isomer of isoleucine (I)',
        'I' => 'no charge, aliphatic, hydrophobic (3.1), isomer of leucine (L)',
        'P' =>
'no charge, aliphatic, promotes turns, cyclic, not hydrophobic (-0.29) ',
        'F' => 'no charge, aromatic, absorbs UV, hydrophobic (2.5)',
        'Y' =>
'weak charge, aromatic, absorbs UV, hydrogen bonding, not hydrophilic (0.08) ',
        'W' =>
'largest amino acid and rarest amino acid, no charge, aromatic, absorbs UV, hydrogen bonding, hydrophobic (1.5)',
        'S' => 'no charge, polar, hydrogen bonding, hydrophilic (-1.1)',
        'T' => 'no charge, polar, hydrogen bonding, hydrophilic (-0.75)',
        'N' =>
'amide of Aspartate (D), polar, hydrogen bonding, no charge, hydrophilic (-2.7)',
        'Q' =>
'amide of Glutamate (E), polar, hydrogen bonding, no charge, hydrophilic (-2.9)',
        'C' =>
'Sulfur analog of Serine(S), weak charge, forms disulfide bonds, not hydrophilic (0.17)',
        'M' =>
'initiator of proteins, containing Sulfur, no charge, hydrophobic (1.1)',
        'D' => 'acidic, negative charge, hydrophilic (-3.0)',
        'E' => 'acidic, negative charge, hydrophilic (-3.0)',
        'H' =>
'imidizole in side chain, basic, reactive, weak positive charge, hydrophilic (-1.7)',
        'K' =>
'amine in side chain, basic, reactive, strong positive charge, hydrophilic (-4.6)',
        'R' =>
'guanidinium side chain, basic, strongest positive charge, hydrophilic (-7.5)',
        '*' => 'stop',
        '-' => 'deletion',
        '?' => 'elongation'
    );
    ...
    for ( my $i = 0 ; $i < length($sequence_protref) ; $i++ ) {
        ...
        if ( defined $mut_hash_aa{ $nhap }{ $pos } ) {
            ...
            if ( $mut_hash_aa{ $nhap }{ $pos }{ "phase" } eq "missing" ) {
                ...
                && $mut_hash_aa{ $nhap }{ $pos }{ "phase" } eq "yes" )
            {
                $color = "blue2";
        ...
        if ( $aaref eq $aamut ) {
            ...
            if ( ( $pos - 1 ) == 0 ) {
                $hash_mut_type{   ( $pos - 1 ) }{ "mut" } =
                  $hash_mut_type{ ( $pos - 1 ) }{ "mut" } . $aamut;
            ...
            if ( defined $hash_mut_type{ $pos - 1 }{ "type" } ) {
                if ( $hash_mut_type{ $pos - 1 }{ "type" } eq "mismatch"
...
#print $k." position mutation \n".$hash_mut_type{$k}{"type"}." type \n".$hash_mut_type{$k}{"frameshift"}." frameshift\n";
                if (   $hash_mut_type{ $k }{ "frameshift" } != 0
                    && $hash_mut_type{ $k }{ "frameshift" } != -1 )
                {
                    $fsize = $hash_mut_type{ $k }{ "frameshift" };
                }
                if (   $hash_mut_type{ $k }{ "frameshift" } == -1
                    || $hash_mut_type{ $k }{ "frameshift" } == 0 )
                {
                    if ( $fsize != 0 ) {
                        ...
                        && $hash_mut_type{ $k }{ "frameshift" } != -1 )
                    {
                        $info_haplo =
                          ...
                          . $k . ":"
                          . $hash_mut_type{ $k }{ "mut" } . ":"
                          . $hash_mut_type{ $k }{ "type" } . ":"
                          . $hash_mut_type{ $k }{ "symbol" } . ":"
                          . $hash_mut_type{ $k }{ "color" };
                        $haplo =
                            $hash_mut_type{ $k }{ "ref" } . ":"
                          . $k . ":"
...
#	print HTML "<br><li>hap 1 ".$chr.":".$chr.$m.":".$mut_hash{$chr}{$m}{'hap1'}{'nt'}." amino acid position ".$mut_hash{$chr}{$m}{'hap1'}{'amino'}." \n</li><br><blockquote>";
                    ...
                    $stop_gen_pos = "yes";
                    while ($hash_mut_type1{ $frame }{ "type" } ne "elongation"
                        && $hash_mut_type1{ $frame }{ "type" } ne "stop"
                        && $frame != $aa_pos )
                    {
                        $frame++;
                    }
...
#print HTML "<br><li>".$mut_hash{$chr}{$hash_mut_type1{$m}{"genomique"}}{'hap1'}{'vcf'}." amino acid position ".$m." \n</li><br><blockquote>";
                        print HTML
"Frameshift mutation that results in protein elongation <br>";
                    }
                    elsif ( $hash_mut_type1{ $frame }{ "type" } eq "stop" ) {

#print HTML "<br><li>".$mut_hash{$chr}{$hash_mut_type1{$m}{"genomique"}}{'hap1'}{'vcf'}." amino acid position ".$m." \n</li><br><blockquote>";
                        print HTML
"Frameshift mutation that results in a premature stop codon at position "
                          . $frame . "<br>";
                    }
                    elsif ( $frame == $aa_pos ) {
                        print HTML
"Frameshift mutation with no elongation of protein or premature stop codon <br>";
                    }
                }
                    ...
                    my $aapref =
                      aa2properties( $hash_mut_type1{ $m }{ "ref" } );
                    my $aapmut =
                      aa2properties( $hash_mut_type1{ $m }{ "mut" } );

#print HTML "<br><li>".$mut_hash{$chr}{$hash_mut_type1{$m}{"genomique"}}{'hap1'}{'vcf'}." amino acid position ".$m." \n</li><br><blockquote>";
                    print HTML $hash_mut_type1{ $m }{ "ref" } . "=>"
                      . $hash_mut_type1{ $m }{ "mut" } . " "
                      . $hash_mut_type1{ $m }{ "type" }
                      . " mutation<br>";
                    print HTML "Change from "
                      . $aapref
                      . "amino acid to "
                      . $aapmut . "<br>";
                }
                  ...
                  )
                {
                      ...
                      )
                    {
                        ...
                        print HTML "<li>"
                          ...
                          { $hash_mut_type1{ $m }{ "genomique" } }
                          { $clin_info }{ "CLNDBN" }
                          . " <a href=\"https://www.ncbi.nlm.nih.gov/clinvar/variation/"
                          . $clin_hash{ $chr }
                          { $hash_mut_type1{ $m }{ "genomique" } }
                          { $clin_info }{ "ID" } . "\">"
                          . "see clinvar"
...
#print $hash_mut_type2{$mut_hash{$chr}{$m2}{'hap2'}{'amino'}}{"ref"}.">".$hash_mut_type2{$mut_hash{$chr}{$m2}{'hap2'}{'amino'}}{"mut"}." ".$hash_mut_type2{$mut_hash{$chr}{$m2}{#'hap2'}{'amino'}}{"type"}." mutation<br>\n";
                    ...
                    $stop_gen_pos2 = "yes";
                    while ($hash_mut_type2{ $frame2 }{ "type" } ne "elongation"
                        && $hash_mut_type2{ $frame2 }{ "type" } ne "stop"
                        && $frame2 != $aa_pos )
                    {
                        $frame2++;
                    }
...
"Frameshift mutation with no elongation of protein or premature stop codon <br>";
                    ...
                    my $aapref =
                      aa2properties( $hash_mut_type2{ $m2 }{ "ref" } );
                    my $aapmut =
                      aa2properties( $hash_mut_type2{ $m2 }{ "mut" } );

#print HTML "<br><li>".$mut_hash{$chr}{$hash_mut_type1{$m}{"genomique"}}{'hap1'}{'vcf'}." amino acid position ".$m." \n</li><br><blockquote>";
                    print HTML $hash_mut_type2{ $m2 }{ "ref" } . "=>"
                      . $hash_mut_type2{ $m2 }{ "mut" } . " "
                      . $hash_mut_type2{ $m2 }{ "type" }
                      . " mutation<br>";
                    print HTML "Change from "
                      . $aapref
                      . "amino acid to "
...
#}
    ...
    print HTML "<br><object class=\"bigpdf\" data=\""
      . $pdf
      . "\" type=\"application/pdf\">alt : <a href=\""
      . $pdf
      . "\">protein.pdf</a></object>";
    print HTML
"<div w3-include-html=\"http://lysine.univ-brest.fr/GEMPROT/footer.html\" id=\"gemprotfooter\" class=\"gemprotfooter\"></div>";
...
sub get_url_pfam {
    ...
    while ( $pfam !~ m/^\</ ) {
        $pfam =
`curl --silent -LH 'Expect:' -F output=xml 'http://pfam.xfam.org/family/Piwi/acc'`;
    ...
    if ( @genedomain == 0 ) {
        while ( @result == 0 ) {
            @url =
`curl --silent -LH 'Expect:' -F seq='<$sequence_gene_prot' -F output=xml 'http://pfam.xfam.org/search/sequence'`;
...
#printf "curl --silent -LH 'Expect:' -F seq='<$sequence_gene_prot' -F output=xml 'http://pfam.xfam.org/search/sequence'\n";
        ...
        while ( $pfam !~ m/^\</ ) {
            $pfam =
`curl --silent -LH 'Expect:' -F output=xml 'http://pfam.xfam.org$res_url'`;
        ...
        if ( $pfam =~ m/matches/ ) {
            ...
            foreach my $match (@entry4) {
                ...
                foreach my $loc (@entry5) {
                    ...
                    $prot_domain{ $start }{ "Start" } =
                      $loc->getAttribute('ali_start');
                    $prot_domain{ $start }{ "End" } =
                      $loc->getAttribute('ali_end');
                    $prot_domain{ $start }{ "Domain" } =
                      $match->getAttribute('id');
                    $prot_domain{ $start }{ "Accession" } =
...
sub domain_file {
    ...
    {
        print DFILE $gene . "\t"
          . $prot_domain{ $key }{ "Domain" } . "\t"
          . $prot_domain{ $key }{ "Start" } . "\t"
          . $prot_domain{ $key }{ "End" } . "\n";
    }
    close DFILE;
}
...
sub get_chr {
    ...
    if ( $vcf =~ /.gz$/ ) {
        open( IN,
            "zcat $vcf | head -2000 | grep -v \"^#\" | head -1 | cut -f1 | " )
          || die "can not open pipe to $vcf";
    }
    else {
        open( IN,
            "cat $vcf | head -2000 | grep -v \"^#\" | head -1 | cut -f1 | " )
          || die "can not open pipe to $vcf";
    }
    my $header = <IN>;
    if ( $header =~ "^chr" ) {
...
sub get_sample {
    ...
    my @elmnts;
    if ( $vcf =~ /.gz$/ ) {
        open( IN, "zcat $vcf | head -2000 | grep \"^#\" | tail -1 | " )
          || die "can not open pipe to $vcf";
    }
    else {
        open( IN, "cat $vcf | head -2000 | grep \"^#\" | tail -1 | " )
          || die "can not open pipe to $vcf";
    }
    my $header = <IN>;
    @elmnts = split( /\t/, $header );
...
sub parsevcf2 {
    my (
        $vcf,      $seq_hash, $seq_hash1, $seq_hash2, $seq_ref1,
        $seq_ref2, $sample,   $clin,      $gene
    ) = @_;
    if ( $vcf =~ /.gz$/ ) {
 ...
 #print "##########Summary of nucleotide mutations present in $gene sequence\n";
    foreach my $line (@variant) {
        ...
        if ( $line =~ "^#CHROM" ) {
            ...
            if ( $num_col == 0 ) {
                printf
"##########ERROR MESSAGE : sample $sample doesn't exist in the vcf\n";
                return (
                    \%seq_hash_hap1, \%seq_hash_hap2, \%seq_hash_ref1,
                    \%seq_hash_ref2, \%mut_hash,      \%mut_hash_pos,
                    $mut_nb1,        $mut_nb2
                );
            }
        }
...
# regex for ALT and REF contain only nucleotide (ATGC) and add "," for billelic multiallelic site
      ...
      # print $fich1[3]." ".$fich1[4]." position $fich1[1] is ATGC variation\n";
                            ...
                            for ( $i = 0 ; $i < length($ALT) ; $i++ ) {
                                ...
                                if ( $i <= ( ( length( $fich1[3] ) ) - 1 ) ) {
                                    ...
                                    $mut_hash{ $fich1[ 0 ] }
                                      ...
                                      . $ALT;
                                    $mut_hash{ $fich1[ 0 ] }
                                      { $variant_pos + $i }{ "hap1" }{ "nt" } =
                                      $REF_nt[$i] . ">" . $ALT_nt[$i];
                                    $mut_hash{ $fich1[ 0 ] }
                                      { $variant_pos + $i }{ "hap1" }{ "amino" }
                                      = $seq_hash_ref{ $fich1[ 0 ] }
                                      { $variant_pos + $i }{ "amino" };
                                    $mut_hash{ $fich1[ 0 ] }
                                      { $variant_pos + $i }{ "hap1" }{ "exon" }
                                      = $seq_hash_ref{ $fich1[ 0 ] }
                                      { $variant_pos + $i }{ "exon" };
                                    $mut_hash{ $fich1[ 0 ] }
                                      { $variant_pos + $i }{ "hap1" }
                                      { "nt_pos" } =
                                      $seq_hash_ref{ $fich1[ 0 ] }
                                      { $variant_pos + $i }{ "nt_pos" };

                                    #}
                                }
                                else {
                                    if ( !defined $mut_hash{ $fich1[ 0 ] }
                                        { $variant_pos + $i_inf }{ "hap1" }
                                        { "nt" } )
                                    {
                                        $mut_hash{ $fich1[ 0 ] }
                                          { $variant_pos + $i_inf }{ "hap1" }
                                        ...
                                        $mut_hash{ $fich1[ 0 ] }
                                          ...
                                          { "exon" } =
                                          $seq_hash_ref{ $fich1[ 0 ] }
                                          { $variant_pos + $i_inf }{ "exon" };
                                        $mut_hash{ $fich1[ 0 ] }
                                          { $variant_pos + $i_inf }{ "hap1" }
                                          { "nt_pos" } =
                                          $seq_hash_ref{ $fich1[ 0 ] }
                                          { $variant_pos + $i_inf }{ "nt_pos" };
                                    }
                            ...
                            for ( $i = 0 ; $i < length($ALT) ; $i++ ) {
                                ...
                                if ( $i <= ( length( $fich1[3] ) - 1 ) ) {
                                    ...
                                    $mut_hash{ $fich1[ 0 ] }
                                      ...
                                      . $ALT;
                                    $mut_hash{ $fich1[ 0 ] }
                                      { $variant_pos + $i }{ "hap2" }{ "nt" } =
                                      $REF_nt[$i] . ">" . $ALT_nt[$i];
                                    $mut_hash{ $fich1[ 0 ] }
                                      { $variant_pos + $i }{ "hap2" }{ "amino" }
                                      = $seq_hash_ref{ $fich1[ 0 ] }
                                      { $variant_pos + $i }{ "amino" };
                                    $mut_hash{ $fich1[ 0 ] }
                                      { $variant_pos + $i }{ "hap2" }{ "exon" }
                                      = $seq_hash_ref{ $fich1[ 0 ] }
                                      { $variant_pos + $i }{ "exon" };
                                    $mut_hash{ $fich1[ 0 ] }
                                      { $variant_pos + $i }{ "hap2" }
                ...
                printf
"##########ERROR MESSAGE : print $fich1[3]>$fich1[4] position $fich1[1] not simple mutation\n";
            }
        }
    }
    close IN;
    ...
    return (
        \%seq_hash_hap1, \%seq_hash_hap2, \%seq_hash_ref1, \%seq_hash_ref2,
        \%mut_hash,      \%mut_hash_pos,  $mut_nb1,        $mut_nb2
    );
}
